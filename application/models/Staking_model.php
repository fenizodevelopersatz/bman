...
class Staking_model extends CI_Model
{
    ...
    /**
     * Per-package rows behind the Lock Wallet detail panel — only currently
     * locked rows (same predicate as lockWalletBalance()), not full staking
     * history. Sourced from user_stakes directly (not staking_swap_orders /
     * Recent Staking Activity) so every locked package appears here
     * regardless of which purchase path created it, and so Maturity Date /
     * Remaining Days are always available — those aren't reliably present on
     * the swap-order view (only populated when a roi_staking_management
     * record exists, which the legacy internal purchase path never creates).
     */
    public function lockWalletDetail($userId)
    {
        // Defensive: is_special can be missing if this environment's DB was
        // re-imported from a dump that predates 2026-07-24_special_offer.sql.
        $hasSpecial = $this->db->field_exists('is_special', 'user_stakes');
        $select = 'us.id AS stake_id, sp.name AS package_name, us.plan_code, us.stake_amount,'
                . ' us.start_date, us.maturity_date, us.status,'
                . ($hasSpecial ? ' us.is_special,' : ' 0 AS is_special,')
                . ' DATEDIFF(us.maturity_date, CURDATE()) AS remaining_days';
        $rows = $this->db->select($select, false)
            ->from('user_stakes us')
            ->join('staking_packages sp', 'sp.id = us.package_id', 'left')
            ->where('us.user_id', (int)$userId)
            ->where_in('us.status', $this->lockWalletStatuses)
            ->where('us.maturity_date >', date('Y-m-d'))
            ->order_by('us.maturity_date', 'ASC')
            ->limit(200)->get()->result_array();

        $planLabels = ['fixed' => 'Fixed', 'regular' => 'Regular', 'combo' => 'Combo'];
        foreach ($rows as &$r) {
            $r['stake_amount']   = (float) $r['stake_amount'];
            $r['locked_amount']  = $r['stake_amount']; // whole principal, always — no partial-unlock mechanism exists
            $r['remaining_days'] = max(0, (int) $r['remaining_days']);
            $r['plan_label']     = $planLabels[$r['plan_code']] ?? ucfirst((string) $r['plan_code']);
            $r['package_type']   = !empty($r['is_special']) ? 'Special Offer' : 'Regular'; // wording matches _staking_packages.php
            $r['status_label']   = $r['status'] === 'processing' ? 'Processing' : ucfirst((string) $r['status']);
            $r['package_name']   = $r['package_name'] ?: ('Package #' . $r['stake_id']);
        }
        return $rows;
    }
}
