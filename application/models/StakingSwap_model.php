    ...
    public function detectBmanTransfer($order_id)
    {
        ...
        foreach ($result['result'] as $tx) {
            ...
            if ($from === strtolower($order['admin_address']) &&
                ...
                // Record the detected hash only — do NOT flip status or create the
                // stake/credit wallets here. StakingPurchasecron.php's own polling
                // (_stepBman()'s _verifyLeg()) is the single, shared completion path;
                // it picks this bman_tx_hash up on its next pass and finishes the
                // order the same way every order is finished. Previously this method
                // independently completed the order itself (_completeStakePurchase()),
                // racing against the cron for the same order — whichever ran first
                // decided which set of business rules applied. Removed entirely
                ...
                log_message('info', "BMAN transfer detected for order {$order_id}: {$tx_hash} — handed off to StakingPurchasecron for completion");
                return true;
            }
        }
