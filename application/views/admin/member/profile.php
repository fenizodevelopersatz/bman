...
(() => {
  ...
  async function loadMatching(){
    ...
    try{const j=await getJson(MATCHING_URL);exportRows.matching=j.data;el.className='';el.innerHTML=`<div class="mp-metric-grid">${Object.entries(j.summary).map(([k,v])=>metric(k.replaceAll('_',' '),v)).join('')}</div><div class="mp-head mt-6 mb-0"><h3>Matching History</h3></div><div class="mp-table-wrap"><table class="mp-table"><thead><tr><th>Date</th><th>Left Volume</th><th>Right Volume</th><th>Matched Volume</th><th>Carry Forward</th><th>Bonus %</th><th>Earning Amount</th><th>Staking Amount</th><th>Status</th></tr></thead><tbody>${j.data.length?j.data.map(r=>`<tr><td>${esc(r.created_at)}</td><td>${num(r.left_before)}</td><td>${num(r.right_before)}</td><td>${num(r.matched_volume)}</td><td>${num(Math.abs(Number(r.left_before)-Number(r.right_before)))}</td><td>${num(r.total_percent)}%</td><td>${num(r.earning_amount)}</td><td>${num(r.staking_amount)}</td><td>Credited</td></tr>`).join(''):empty(9,'No binary matching history found.')}</tbody></table></div>`}catch(e){el.className='mp-empty';el.textContent=e.message}
  }
  async function loadRanks(){
    const el=document.getElementById('rank-content');
    try{const j=await getJson(RANKS_URL);exportRows.ranks=j.data;el.className='mp-timeline';el.innerHTML=j.data.length?j.data.map(r=>`<article class="mp-rank-step">${r.badge_image?`<img src="${esc(r.badge_image)}" onerror="this.style.display='none'" alt="" style="width:34px;height:34px;object-fit:contain">`:''}<b>${esc(r.current_rank||'—')}</b><div class="mp-node-meta">${esc(r.achieved_at)}</div><div class="mp-node-meta">Previous: ${esc(r.previous_rank||'Default')}</div><div class="mp-node-meta">Rank volume: ${num(r.achieved_volume)}</div><div class="mp-node-meta">${r.earning_volume!=null?`Earning ${num(r.earning_volume)} · Staking ${num(r.staking_volume)}`:'—'}</div><div class="mp-node-meta">Reward: ${num(r.reward_amount)} BMAN</div><div class="mp-node-meta">Approval: ${r.source==='admin'?'Admin approved':'Auto approved'}</div>${r.certificate_pdf?`<a href="${esc(r.certificate_pdf)}" download>Certificate</a>`:''}<div class="mp-status"><span class="mp-dot"></span>${esc(r.reward_status||'Achieved')}</div></article>`).join(''):'<div class="mp-empty">No rank achievements found.</div>'}catch(e){el.className='mp-empty';el.textContent=e.message}
  }
