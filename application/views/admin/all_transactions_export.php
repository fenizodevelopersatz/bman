<?php
/**
 * Export template for All Transactions.
...
<div class="meta">
    BMAN All Transactions · generated <?php echo date('d M Y H:i'); ?> · <?php echo count($rows); ?> row(s)
</div>

<?php if (empty($rows)): ?>
    <div class="empty">No transactions match the current filters.</div>
<?php else: ?>
<table>
    <thead>
    ...
    <?php foreach ($rows as $r): ?>
        ...
        <?php foreach (array_keys($cols) as $k):
            $v = $r[$k] ?? '';
            $num = is_numeric($v) && in_array($k, ['amount', 'balance_after'], true);
        ?>
            <td class="<?php echo $num ? 'num' : ''; ?>">
                <?php echo $num ? number_format((float)$v, 8) : html_escape((string)$v); ?>
            </td>
        <?php endforeach; ?>
        </tr>
    <?php endforeach; ?>
