<?php
/**
 * Member ▸ my rank certificate, print-ready.
 *
 * Standalone page — deliberately NOT wrapped in the member chrome so the browser
 * print dialog produces a clean artefact. Rendered as print-ready HTML rather
 * than a generated PDF binary: this project vendors no PDF library, and
 * Print → Save as PDF produces the same file with no new dependency.
 *
 * Access is scoped in the controller: the certificate is looked up by number AND
 * user_id, so a member can only ever open their own.
 *
 * @var array $c  row from Rankreward_model::certificate()
 */
...
<head>
    ...
    <title><?= htmlspecialchars($c['certificate_no']); ?> — <?= htmlspecialchars($c['rank_name']); ?></title>
    <style>
        * { box-sizing: border-box; }
        body { margin:0; background:#eef1f5; font-family:Georgia,'Times New Roman',serif; color:#1a1a1a; }
        .sheet { width:297mm; height:210mm; margin:24px auto; background:#fff; position:relative;
                 padding:18mm; box-shadow:0 8px 32px rgba(0,0,0,.14); }
        .frame { position:absolute; inset:10mm; border:2px solid <?= htmlspecialchars($colour); ?>; }
        .frame:after { content:''; position:absolute; inset:4mm; border:1px solid <?= htmlspecialchars($colour); ?>66; }
        ...
        .bar { position:absolute; top:0; left:0; right:0; height:6mm; background:<?= htmlspecialchars($colour); ?>; }
        .toolbar { text-align:center; margin:18px 0; font-family:system-ui,sans-serif; }
        .toolbar button, .toolbar a { font:inherit; font-size:13px; padding:8px 18px; margin:0 4px;
            border-radius:6px; border:1px solid #c3ccd8; background:#fff; color:#222; cursor:pointer; text-decoration:none; }
        .toolbar button { background:#6E56CF; border-color:#6E56CF; color:#fff; }
        ...
        @media (max-width:320mm) {
            .sheet { width:100%; height:auto; min-height:210mm; }
        }
    </style>
</head>
...
<body>
    ...
    <div class="toolbar">
        ...
        <a href="<?= base_url('user/rank-reward'); ?>">Back to Rank &amp; Rewards</a>
    </div>
    ...
    <div class="sheet">
        ...
        <div class="inner">
            ...
            <?php if (!empty($c['badge_image'])): ?>
                <img class="badge-img" src="<?= base_url($c['badge_image']); ?>" alt="">
            <?php else: ?>
                <div class="dot"></div>
            <?php endif; ?>
        ...
        <div class="feet">
            <div class="foot">
                <b><?= htmlspecialchars($c['certificate_no']); ?></b>
                Issued <?= date('d F Y', strtotime($c['generated_date'])); ?>
            </div>
            <div class="sig">
