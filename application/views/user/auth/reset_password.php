      ...
      <div class="lpx-form-inner">
        ...
        <form class="form w-100" novalidate="novalidate" id="kt_password_reset_form" method="post">
            ...
            <div class="fv-row mb-8">
                <input type="password" placeholder="New Password" name="password" class="form-control bg-transparent" required />
            ...
            <div class="fv-row mb-8">
                <input type="password" placeholder="Confirm Password" name="password_confirm" class="form-control bg-transparent" required />
            </div>
            ...
            <div class="d-flex flex-wrap justify-content-center gap-3 pb-lg-0">
                ...
                <a href="<?php echo base_url(); ?>user/in" class="btn btn-light">Back to Login</a>
            </div>
        </form>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    submitBtn.addEventListener('click', function(e) {
        ...
        const password_confirm = form.querySelector('input[name="password_confirm"]').value.trim();
        const button = this;
        ...
        if (!password || !password_confirm) {
            msgEl.textContent = 'Please enter both password fields';
        ...
        fetch(base_url + 'user/reset-password?token=<?php echo htmlspecialchars($token); ?>', {
            method: 'POST',
            body: formData
        })
        .then(r => r.json())
        .then(res => {
            ...
            msgEl.textContent = res.message || (res.status ? 'Password reset successfully' : 'Something went wrong. Please try again.');
            msgEl.style.display = 'block';
            ...
            if (res.status) {
                ...
                setTimeout(() => window.location.href = base_url + 'user/in', 2000);
            } else {
                msgEl.style.background = '#fee2e2';
                msgEl.style.color = '#991b1b';
                msgEl.style.border = '1px solid #fca5a5';
