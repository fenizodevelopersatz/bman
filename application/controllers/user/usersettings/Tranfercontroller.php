...
class  Tranfercontroller extends CI_Controller {
    ...
    */
       public function index(){
        ...
        $this->data['redirect_url'] = base_url()."user/tranfer";
        
		$this->load->view('user/wallet/wallet_management',$this->data);
 ...
 /*
    ...
    */
    public function internel_swap(){
		...
		$this->load->view('user/wallet/swap_management',$this->data);
    ...
    */
    ...
    public function myorders(){
        ...
        $this->data['card_title'] = "My Orders";
        $this->data['users'] = $this->db->query("SELECT * FROM users where status = '1' ")->result();
        $this->data['currency_info'] = currency_info();
        $this->data['token_info'] = currency_info();
        $this->data['action'] = base_url()."make-swap-post";
		...
		$this->load->view('user/shop/myorder',$this->data);
    }


   public function fetch_user_orders()
    {
        $draw = $this->input->get('draw');
        $start = $this->input->get('start');
        $length = $this->input->get('length');
        ...
        $total_records = $this->Product_model->count_all_orders($status, $category);
        $orders = $this->Product_model->get_orders($length, $start, $status, $category);
        ...
        foreach ($orders as $order) {
            ...
            $items = $this->db->get_where('order_items', ['order_id' => $order['id']])->result_array();
            ...
            foreach ($items as $item) {
                ...
                $products_html .= '
                    <div class="mb-2">
                        <strong>' . ($product ? $product->name : 'Product Deleted') . '</strong><br/>
            ...
            $shipping_info = $shipping ? "
                ...
                </div>" : "<span class='text-muted'>No Shipping Info</span>";
            ...
            $invoice_btn = '
                <a href="' . base_url('download-invoice/' . $order['id']) . '" class="btn btn-sm btn-light-primary">
                    <i class="fa fa-download"></i> Invoice
                </a>';

            $expanded_html = '
                <div class="row">
                    <div class="col-md-6">' . $products_html . '</div>
                    <div class="col-md-6">' . $payment_info . $shipping_info . $invoice_btn . '</div>
                </div>';

            $data[] = [
                'RecordID' => $i++,
                'OrderID' => '<a href="#">#' . $order['order_code'] . '</a>',
                'Created' => '<span>' . date('d M Y, h:i A', strtotime($order['created_at'])) . '</span>',
                'Total' => '<span class="text-gray-800 fw-bold">$' . number_format($order['total_amount'], 2) . '</span>',
                'Profit' => '<span class="text-success fw-bold">$' . number_format($order['commission_amount'], 2) . '</span>',
                'Status' => '<span class="badge badge-light-' . $this->get_status_color($order['payment_status']) . '">' . ucfirst($order['payment_status']) . '</span>',
                'Action' => '
                <div class="d-flex justify-content-end">
                    <a href="' . base_url('user/shop/view-order/' . $order['id']) . '" class="btn btn-sm btn-info me-2">
   ...
   public function view_order($order_id){
    ...
    $this->data['card_title'] = "View Orders";
    ...
    $this->db->select('oi.*, p.name as product_name, p.product_image, p.sku, p.commission, p.category_id');
    $this->db->from('order_items oi');
    $this->db->join('products p', 'oi.product_id = p.id', 'left');
    $this->db->where('oi.order_id', $order_id);
    $order_items = $this->db->get()->result();

    // Category name on each item
    foreach ($order_items as &$item) {
        $cat = $this->db->get_where('product_categories', ['id' => $item->category_id])->row();
        $item->category_name = $cat ? $cat->name : 'N/A';
    }

    // ===== NEW: latest shipment (single row) =====
    $latest_shipment = $this->db->query("
        SELECT os.*
    ...
    // ===== NEW: history (for timeline) =====
    $history = $this->db
      ->order_by('id','DESC')
      ->get_where('order_status_history', ['order_id'=>$order_id])
      ->result();

    // ===== NEW: derived ship_status fallback =====
    if ($latest_shipment) {
        $ship_status = $latest_shipment->status;
    } else {
        $ship_status = ($order->payment_status==='paid' ? 'paid' :
                       ($order->payment_status==='failed' ? 'failed' : 'placed'));
    }
    ...
    $this->data['latest_shipment']  = $latest_shipment;   // NEW
    $this->data['history']          = $history;           // NEW
    ...
    $this->load->view('user/shop/view-order',$this->data);
}
