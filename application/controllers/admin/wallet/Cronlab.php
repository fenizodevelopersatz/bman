    ...
    public function index()
    {
        ...
        $data = [
            ...
            'jobs' => [
                ...
                ['key' => 'member_bulk_bman', 'label' => 'Member Bulk Upload — Opening BMAN', 'type' => 'wallet', 'endpoint' => 'member-bulk-bman-cron', 'method' => 'GET', 'description' => 'Sends the opening BMAN balance to members created by Members Management ▸ Bulk Upload. The import creates the accounts and leaves each row that carried a bman amount at bman_status=pending; this drains that queue from the Treasury wallet, up to member_bulk_upload_settings.max_batch_size per run. This is NEW money — no internal ledger wallet is debited and the members\' dashboard balances are untouched, only their real on-chain balance. Disabled and dry-run by default — flip member_bulk_upload_settings.enabled / dry_run to go live. A failed send stays failed until an admin re-queues it from the batch detail page. Run every few minutes.'],
            ],
        ];
