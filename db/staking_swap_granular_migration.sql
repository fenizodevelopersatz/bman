-- =============================================================================
-- Migration: Add granular per-transaction cron status tracking
-- Version: 1.1
-- Date: 2026-07-09
-- Description: Track each transaction step independently with its own cron status
-- =============================================================================
...
SELECT '✅ Granular cron status migration completed!' as message;
...
WHERE TABLE_SCHEMA = DATABASE()
  ...
  AND COLUMN_NAME LIKE '%cron_status%' OR COLUMN_NAME LIKE 'bman_%_tx_hash' OR COLUMN_NAME LIKE 'bonus_tx_hash'
ORDER BY ORDINAL_POSITION;

COMMIT;

SELECT CONCAT('Migration v1.1 completed at ', NOW()) as completion_message,
       'Each TX step now has independent cron status tracking for granular retry capability' as details;
